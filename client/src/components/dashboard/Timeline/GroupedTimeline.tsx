...
export const GroupedTimeLine: FC<{
  ...
  takenSemesters: { year: number; term: string }[];
}> = memo(
  ({ programGrades: programGradesProp, filteredGrades, takenSemesters }) => {
                  ...
                  <circle
                    ...
                    fill={
                      checkExplicitSemester({
                        term: takenSemesters[key]?.term ?? "",
                        year: takenSemesters[key]?.year ?? 0,
                      })
                        ? config.TIMELINE_EXPLICIT_CIRCLE_COLOR
                        : config.CUMULATED_GRADE_COLOR
                    }
                    css={transitionCSS}
                  />
                </TimeLineTooltip>
